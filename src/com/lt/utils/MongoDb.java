...
public class MongoDb {
     ...
     * @return 返回集合列表 
     */  
    public static List<Document> findBy(Bson filter,int page,int size,String sort) {  
        List<Document> results = new ArrayList<Document>();  
        //FindIterable<Document> iterables = collection.find(filter)
        //		.sort(Sorts.ascending(sort)).skip((page-1)*size).limit(size);  
        FindIterable<Document> iterables = collection.find(filter)
        		.sort(Sorts.ascending(sort)).limit(size);  
        MongoCursor<Document> cursor = iterables.iterator();  
